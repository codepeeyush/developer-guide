...
import { type ComponentProps, createContext, type PointerEvent, type ReactNode, type RefObject, use, useEffect, useMemo, useRef, useState } from "react";
...
export function useSidebar(): SidebarContext {
  ...
  if (!ctx) throw new Error("Missing SidebarContext, make sure you have wrapped the component in <DocsLayout />.");
...
export function SidebarContent({
  ...
  children: (state: { ref: RefObject<HTMLElement | null>; collapsed: boolean; hovered: boolean; onPointerEnter: (event: PointerEvent) => void; onPointerLeave: (event: PointerEvent) => void }) => ReactNode;
}) {
...
export function SidebarViewport(props: ScrollAreaProps) {
  return (
    <ScrollArea {...props} className={cn("min-h-0 flex-1", props.className)}>
      <ScrollViewport
        className="px-4 overscroll-contain pb-10 md:pb-20"
        style={
          {
            maskImage: "linear-gradient(to bottom, transparent, foreground 12px, foreground calc(100% - 12px), transparent)",
          } as object
        }
      >
        {props.children}
...
export function SidebarSeparator(props: ComponentProps<"p">) {
  ...
  return (
    <p {...props} className={cn("sticky top-0 w-full z-10 md:bg-background/95 bg-fd-muted backdrop-blur-sm", "inline-flex items-center gap-2 mb-1.5 px-2 py-2 mt-4 empty:mb-0", depth === 0 && "first:mt-0", props.className)}>
      {props.children}
    </p>
  );
}
...
export function SidebarItem({
  ...
  return (
    <Link ref={ref} data-active={active} prefetch={prefetch} {...props}>
      {icon ?? (props.external ? <HugeiconsIcon icon={LinkSquare01Icon} /> : null)}
      {children}
    </Link>
  );
}
...
export function SidebarFolder({
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} disabled={!collapsible} {...props}>
      <FolderContext value={useMemo(() => ({ open, setOpen, depth, collapsible }), [collapsible, depth, open])}>{children}</FolderContext>
    </Collapsible>
  );
}
  ...
  if (collapsible) {
    return (
      <CollapsibleTrigger {...props}>
        {children}
        <HugeiconsIcon icon={ArrowDown01Icon} data-icon className={cn("ms-auto transition-transform", !open && "-rotate-90")} />
      </CollapsibleTrigger>
    );
  }
...
export function SidebarFolderLink({ children, ...props }: LinkProps) {
  ...
  return (
    ...
    >
      {children}
      {collapsible && <HugeiconsIcon icon={ArrowDown01Icon} data-icon className={cn("ms-auto transition-transform", !open && "-rotate-90")} />}
    </Link>
  );
}
