...
import { type ComponentProps, createContext, Fragment, type HTMLAttributes, type PointerEvent, type ReactNode, use, useMemo, useRef, useState } from "react";
...
export function LayoutBody({ className, style, children, ...props }: ComponentProps<"div">) {
  ...
  const pageCol = "calc(var(--fd-layout-width,92rem) - var(--fd-sidebar-col) - var(--fd-toc-width))";
      ...
      {options.map((option, i) => {
        ...
        return (
          <Link
            ...
            href={url}
            className={cn(
              "inline-flex border-b-2 border-transparent transition-colors items-center pb-1.5 font-medium gap-2 text-fd-muted-foreground text-sm text-nowrap hover:text-fd-accent-foreground [&_svg]:size-4",
...
export function NavbarLinkItem({ item, className, ...props }: { item: LinkItemType } & HTMLAttributes<HTMLElement>) {
  ...
  return (
    <LinkItem item={item} className={cn("text-sm text-fd-muted-foreground transition-colors hover:text-fd-accent-foreground data-[active=true]:text-fd-primary", className)} {...props}>
      {item.text}
    </LinkItem>
  );
}

function NavbarLinkItemMenu({ item, hoverDelay = 50, className, ...props }: { item: MenuItemType; hoverDelay?: number } & HTMLAttributes<HTMLElement>) {
  ...
  return (
    ...
    >
      ...
      <PopoverContent className="flex flex-col p-1 text-fd-muted-foreground text-start" onPointerEnter={onPointerEnter} onPointerLeave={onPointerLeave}>
